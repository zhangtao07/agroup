...
define([
  ...
  var refreshPreviewLater = (function() {
    ...
    if (settings.lazyRendering === true) {
      return _.debounce(refreshPreview, 5);
    }
    return function() {
  ...
  eventMgr.addListener('onSectionsSynced', function(newSectionList) {
    ...
    watcher.noWatch(function() {
      //updateSectionList(newSectionList);
      //syncHighlight();
      //var op = synSectionList(newSectionList);
      //hightSyncSections(op.m, op.r, op.i);
    });
  ...
  window.selectionMgr = selectionMgr;
  ...
  function syncValue(value, cb) {
    watcher.noWatch(function() {
      ...
      range.deleteContents();
      range.insertNode(document.createTextNode(replacement));
      var pos = range.getClientRects()[0];
      range.detach();
  ...
  function updateCursor(top, left) {
    var st = $inputElt.scrollTop();
    this.$cursorElt.css({
      top: st + top, //- $inputOffset.top,
      left: left,
      opacity: 1
    });
    this.$avatarElt.css({
  ...
  editor.syncValueNoWatch = function(value, user) {
    ...
    syncValue(value,function(pos){
    //console.log(offset,v,o);
    //var cursor = selectionMgr.getCoordinates(offset.end);
    //console.log(cursor);
    //var st = crel('span');
    ...
    //var offset = $(st).position();
      ...
      if(pos){
        updateCursor.call(users[user.id],pos.top,pos.left+pos.width);
        clearTimeout(usertime);
        usertime = setTimeout(hideUserCursor.bind(users[user.id]),800);
      }else{
        hideUserCursor.call(users[user.id]);
      }
      //syncContentChanged();
    });
    fileDesc.content = textContent = value;
    eventMgr.onContentSynced(fileDesc, value);
    refreshPreviewLater();
  }
  window.checkContentChange = checkContentChange;

  var testFn = (function() {
    var value = textContent || '## 测试开始';
    var cont = 0;
    return function() {
    ...
    function restoreState(state, selectionStart, selectionEnd) {
      ...
      watcher.noWatch(function() {
        if (textContent != state.content) {
          ...
          var oldcontent = fileDesc.content;
          fileDesc.content = state.content;
          eventMgr.onContentChanged(fileDesc, state.content, oldcontent);
        }
  ...
  function syncContentChanged(){
    window.inputElt = inputElt;
    ...
    if (fileChanged === false) {
      ...
      //eventMgr.onContentChanged(fileDesc, textContent, oldcontent);
  ...
  function syncHighlight() {
    ...
    watcher.noWatch(function() {
      if (fileChanged === true) {
        ...
        var len = newSectionEltList.textContent.length;
        if (insertBeforeSection !== undefined) {
          contentElt.insertBefore(newSectionEltList, insertBeforeSection.elt);
        } else {
          contentElt.appendChild(newSectionEltList);
        }
        if( len < selectionMgr.selectionEnd){
          var endOffset = selectionMgr.selectionEnd + 1;
          selectionMgr.setSelectionStartEnd(endOffset, endOffset);
        }
  ...
  function hightSyncSections(modifiedSections, sectionsToRemove, insertBeforeSection) {
    var newSectionEltList = document.createDocumentFragment();
    modifiedSections.forEach(function(section) {
      highlight(section);
      newSectionEltList.appendChild(section.elt);
      section.elt.className = section.elt.className + ' u-lock';
    });
    watcher.noWatch(function() {
      // Remove outdated sections
      sectionsToRemove.forEach(function(section) {
  ...
  function synSectionList(newSectionList) {

    var modifiedSections = [];
    var sectionsToRemove = [];
    var insertBeforeSection = undefined;
    //var sectionList = [];//newSectionList;
    var ssList = sectionList.slice();
